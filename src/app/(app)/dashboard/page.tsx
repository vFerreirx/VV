...
import { Activity, AlertTriangle, CircleAlert, Factory, Package } from 'lucide-react'
...
import { contarTarefasPendentes, listarTarefasDoPainel } from '../tarefas/actions'
...
export default async function DashboardPage() {
  ...
  const [kpis, opsUrgentes, producao14d, canais, topProdutos, remessas, tarefas, tarefasPendentes] =
    await Promise.all([
      obterKPIs(),
      listarOpsUrgentes(5),
    ...
    <ViewTransition enter="vt-entra-sobe" default="none">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Bem-vindo, {user.nome.split(' ')[0]}.</p>
        </div>
              ...
              <ul className="divide-y">
                {opsUrgentes.map((op) => (
                  <li key={op.id} className="flex items-center gap-3 py-2.5">
                    <Link href={`/ordens/${op.id}`} className="font-mono text-xs hover:underline">
                      {op.numero}
                    </Link>
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm font-medium">{op.produtoNome}</div>
                    ...
                    {op.dataPrevistaFim && (
                      <span
                        className={cn(
                          ...
                          op.atrasada ? 'text-destructive font-medium' : 'text-muted-foreground',
                        )}
                      >
...
function KPICard({
  ...
  return (
    <Card className="vv-lift">
      ...
      <CardContent>
        <div className={cn('text-3xl font-semibold tabular-nums', alerta && 'text-destructive')}>
        ...
        {emRisco.length === 0 ? (
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium">
              ...
              {risco.length > 0 && <span className="text-amber-600">{risco.length} em risco</span>}
            </p>
            <ul className="divide-y">
    ...
    <div className="space-y-3">
      {kpis.distribuicaoStatus.map((d) => {
        ...
        return (
          ...
          >
            <div className="text-muted-foreground truncate">{STATUS_LABEL_CURTO[d.status]}</div>
            ...
            <div className="text-right font-medium tabular-nums">{d.total}</div>
          </div>
        )
      })}
    </div>
