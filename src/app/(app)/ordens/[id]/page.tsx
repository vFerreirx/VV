...
import { OrdemForm, type OrdemFormDefaults } from '@/components/forms/ordem-form'
...
export default async function EditarOrdemPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [ordem, produtos] = await Promise.all([obterOrdem(id), listarProdutosParaOrdem()])
  if (!ordem) notFound()
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="truncate font-mono text-2xl font-semibold">{ordem.numero}</h1>
          <p className="text-muted-foreground truncate text-sm">{ordem.produto.nome}</p>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent className="text-muted-foreground grid grid-cols-2 gap-x-4 gap-y-2 text-sm md:grid-cols-4">
          <Info label="Quantidade" value={`${ordem.quantidade.toLocaleString('pt-BR')} un`} />
          <Info
            label="Criada em"
          ...
          <Info label="Máquina" value={ordem.maquina ? ordem.maquina.nome : '—'} />
        </CardContent>
      </Card>
